...
| [`term/term.c`](term/term.c) | Sub-task 9.6: the character grid a terminal emulator draws — what stands where, where the cursor is, which rows have changed, and the four control characters it acts upon, including the backspace that crosses to the row above because the line editor erases with one. It touches no descriptor, which is what lets the kernel's self-test drive it without a window. `../docs/design/TERMINAL.md`, Section 3. |
