...
| `include/oxys/gfx/client.h` | Sub-task 9.2. The client side of the window manager implemented in `graphics/`: the six window calls as the entry path hands them, the ownership of a window by the process that made it, the release of a process's windows at its ending, and the wake of every program asleep for an event. `docs/design/WINDOWS.md`, Section 10. |
