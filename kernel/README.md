...
| Path | Description |
| ---- | ----------- |
| `kernel.c` | `KernelMain`, the C entry point, now a short driver that calls the phases of `init/` in dependency order; and what every phase writes through: the diagnostic channel, the display's quiet and mode, the command line, the panic, the halt, the boot and power screens, and the power call. |
| `init/` | The phases of the boot, one file each, from the serial line to the session. Described by [`init/README.md`](init/README.md). |
