...
| `mm/table.c` | The growing table behind the process, thread, node, open-file and pipe tables. `GrowingTableGrow`, `GrowingTableReport`. |
...
| `include/oxys/mm/table.h` | The growing table: `GrowingTable`, `GROWING_TABLE_INITIALISER`, `GrowingTableCapacity`, `GrowingTableAt`. |
...
| `include/oxys/gfx/window.h` | Sub-task 9.1. The interface of the window manager implemented in `graphics/`: the table of windows, the stack, the focus, the binding of the pointer to a window by a held button, the frame drawn about each, the event a window's owner reads, and the composition that says what changed. `docs/design/WINDOWS.md`. |
| `include/oxys/gfx/client.h` | Sub-task 9.2. The client side of the window manager implemented in `graphics/`: the six window calls as the entry path hands them, the ownership of a window by the process that made it, the release of a process's windows at its ending, and the wake of every program asleep for an event. `docs/design/WINDOWS.md`. |
...
| `fs/ext2/` | The EXT2 implementation, divided into nine translation units and a private header. It was one file of 4,325 lines until it was divided; `../docs/design/ARCHITECTURE.md` records why, and along which lines. The public interface is unchanged and is `<oxys/fs/ext2.h>`. |
...
| `fs/vfs/` | The virtual filesystem layer, divided into six translation units and a private header. It was one file of 2,355 lines; `../docs/design/ARCHITECTURE.md` records why and along which lines. The public interface is unchanged and is `<oxys/fs/vfs.h>`. |
| `fs/vfs/internal.h` | What those units share: the two fixed and two growing tables the layer's whole state lives in, `VfsFile`, the refusal record and the accounting, and the resolution and node-cache primitives. Beside the implementation and not in `include/oxys/`, because nothing outside `fs/vfs/` may depend upon it. |
...
| `fs/persist.c` | The persistent `/etc`, of 2026-09-23: the volume labelled `oxys-etc` found upon a disk by its whole label, marked clean where a stopped machine left it open, mounted over the ramdisk's `/etc`, seeded with what it lacks without overwriting what it has, and synced and unmounted at the power call. `../docs/storage/PERSIST.md`. |
...
| `include/oxys/fs/persist.h` | `PersistMountByLabel`, `PersistFindByLabel`, `PersistMountDevice`, `PersistRelease` and `PersistEtc`, and why `/etc` is a volume of its own rather than a directory of a larger one bound onto it. |
| `block/` | The generic block-device layer and the buffer cache above it. It is **not** in `../drivers/`, and the reason is the test that directory's own `README.md` applies to the framebuffer: nothing here programs anything. A driver implements an interface this corpus declares; these two declare the interface a driver registers *into*, which is the opposite relation. `../docs/design/ARCHITECTURE.md` records the move. |
...
4. **Every processor schedules; three structures are locked.** Sub-task 6.13
   ...
   `docs/design/ARCHITECTURE.md` requires.
