...
3. The boot-time self-tests are part of the kernel image and are never absent
   ...
   consequences.
4. **The lock exists; it is not yet applied.** Sub-task 6.13 built the ticket
   spinlock, the per-processor area beneath it, the inter-processor interrupt and
   the shootdown; `docs/design/CONCURRENCY.md` is the design. No structure here
   has been put under a lock, there being one thread of control — the frame
   allocator's bitmap and search hint, the arena, the heap, the process and
   thread tables and the dispatch table all still say so in their own headers,
   and sub-tasks 6.14 and 6.15 are what make each contended. Every structure
