...
| `arch/x86_64/` | **Everything here that could not survive a change of processor.** Six subdirectories — `cpu/`, `interrupt/`, `syscall/`, `smp/`, `mm/` and `proc/` — and the rule for admitting a file to one of them is [`arch/README.md`](arch/README.md). The per-file detail stays in the table below rather than being restated there. Note what the directory does **not** contain: the headers. `<oxys/gdt.h>` stays in `include/oxys/` beside the rest of the corpus, so a caller names the interface and never the implementation's location, which is the rule `../drivers/README.md` already states for a device driver. `../docs/design/ARCHITECTURE.md`, Section 2.4, records the boundary and where it is presently untidy. |
...
| `include/oxys/buffer.h` | The interface of the buffer cache implemented in `block/`: obtaining, releasing, dirtying and flushing a cached block. |
| `include/oxys/block.h` | The interface of the generic block-device layer implemented in `block/`: the operations a driver supplies, and the read and write path above them. |
