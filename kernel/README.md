...
| `proc/process.c` | The process control block and the thread structure: the tables, the creation of a process with an address space of its own and a thread with a kernel stack of its own beneath a guard page, the writing of `rsp0` when a thread becomes current, and the record of what a process has had loaded into it. `ProcessInitialise`, `ProcessCreate`, `ProcessDestroy`, `ThreadCreate`, `ThreadDestroy`, `ThreadSetCurrent`, `ProcessCreateUserStack`, `ProcessRecordImage`, `ProcessReport`. |
