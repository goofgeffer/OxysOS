...
| `include/oxys/syscall.h` | The named bits of `RFLAGS` and `SYSCALL_FLAG_MASK` composed from them; the call numbers and the error values, which are this kernel's own; the canonical limit a user argument is tested against; and the interface by which the configuration is read back from the processor and asserted. |
