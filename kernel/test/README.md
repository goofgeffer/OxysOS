...
| `gfx/windows.c` | Phase 9, sub-task 9.1: the window manager upon a screen composed in memory — the stack read from the pixel where two windows overlap, the focus, the routing of keys and movements, the binding by a held button, the drag, the close control, the confinement and the two bounds — and the disc of `graphics/draw.c`; and the **demonstration** the default boot entry presents, three windows a person operates, serviced from the tick. The one file here that holds something other than a test, for the reason `gfx/graphics.c` draws a figure: it is what a person judges. |
