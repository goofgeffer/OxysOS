...
| `storage/vfs.c` | Sub-task 5.8: the virtual filesystem layer, its mounts and its node identity; and `KernelVfsProbeVolume`, which exercises a real volume through the layer. |
...
| `shell/parser.c` | Phase 8, sub-tasks 8.2 and 8.3: the shell's tokeniser and parser, and since 8.3 its assignment words, variables and expansion, asserted against the translation units the shell ships — compiled into this image as the C library's are — with some fifty lines of known tokens and structure: every operator longest first, the `io_number`, the three quotings with the quotes kept, the comment, the subset of the grammar parsed and the remainder refused by name, every bound, and what is incomplete rather than wrong. The shell is then run at privilege level 3 upon a session of eight lines that continues a command across a quote and across a pipe, and is asserted to consume it and end with zero. `../../docs/design/SHELL.md`, Section 9. |
