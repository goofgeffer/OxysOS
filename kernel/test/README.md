...
`docs/project/TESTING.md` records what that target asserts and why
...
below and `../../docs/design/ARCHITECTURE.md`.
...
| `mm/memory.c` | Phase 2: the physical frame allocator, the paging hierarchy, the virtual address allocator and the heap, per-frame reference counting, the resolution of a copy-on-write fault, and the cloning of an address space; since 2026-09-25, the growing table (`KernelVerifyGrowingTable`). |
...
| `storage/vfs.c` | Sub-task 5.8: the virtual filesystem layer, its mounts and its node identity; and `KernelVfsProbeVolume`, which exercises a real volume through the layer. Since 8.6, `KernelVerifyVfsPipes`: the pipe from a caller that cannot sleep, which is everything about it but the sleeping. Since 2026-09-25, `KernelVerifyVfsGrowth`: more open files and pipes than a first chunk holds. |
...
states: the public corpus is what a consumer may depend upon, and
...
`docs/project/TESTING-GRAPHICS.md` records the procedure by which a person
judges it.
...
described in `docs/storage/VFS.md` was found: every assertion in
...
4. **The fixture is one volume geometry**: 1024-byte blocks, one block group, 32
   ...
   real images, which `docs/project/TESTING-SYSTEM.md` records
