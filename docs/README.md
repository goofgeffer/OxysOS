...
| [`VERSIONING.md`](project/VERSIONING.md) | The release naming scheme: one repository, many releases — the ordinal, the point beneath it, the alpha and beta that precede the first ordinal, the name that stands for an ordinal, and the edition that is a variety rather than a successor — the record of releases, which is empty, and the three that are planned. |
