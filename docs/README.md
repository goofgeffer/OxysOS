...
| [`VERSIONING.md`](project/VERSIONING.md) | The release naming scheme: one repository, many releases — the ordinal, the point beneath it, the alpha and beta that precede the first ordinal and recur for no later one, the name that stands for an ordinal, and the edition that is a variety rather than a successor — the record of releases — `Oxys 1 Alpha`, cut 2026-09-16 — and the three that are planned. |
| [`RELEASE-1-ALPHA.md`](project/RELEASE-1-ALPHA.md) | The release notes of `Oxys 1 Alpha`: what it is, what it does, what it does not have, where it was tested, and the checksum of its image. |
...
| [`SHELL.md`](design/SHELL.md) | The shell of Phase 8, one section per sub-task, all seven: the terminal the kernel assembles from the keyboard and the serial line, the line editor and its history above it, the tokeniser and the parser, the built-ins and the working directory, programs run by `fork` and `execve`, redirection, pipelines and the two programs at once they needed, and job control with the signals the terminal delivers — with the editor `micro`, `clear`, the prompt, and six utilities beside them. |
| [`WINDOWS.md`](design/WINDOWS.md) | The window manager of sub-task 9.1 — the stack, the focus, the routing of every key and every movement to the window it belongs to, the frame and the palette it is drawn with — and the client protocol of 9.2, by which a program owns a window through six calls; and the three windows the default boot entry presents, drawn by the first such program, until sub-task 9.5 has a desktop. |
