...
**Phase 5 is complete.** Sub-task 5.1: the superblock of an EXT2 volume
...
Sub-task 5.8 is complete, and with it the phase. Sub-tasks 5.1 to 5.7 built an
EXT2 implementation that can do everything the format admits and that could not
be used: nothing was retained, so a caller wishing to read two files read the
superblock twice; nothing was open, so a file was acted upon by naming it and
there was no position that advanced; and there was one volume, so a machine with
two disks had two unrelated trees and no path that reached both. A virtual
filesystem layer supplies the three, and they are absent together rather than by
coincidence — each is a thing the format has no business knowing.

A filesystem is registered under a name and supplies sixteen operations; four of
them are not optional, because without them nothing can be reached at all, and
the absence of any other is refused where it is called rather than by a call
through a null pointer. The EXT2 binding is a file of its own, and is the only
file in the project that knows both that a node has a reference count and that an
inode has fifteen block pointers.

Three properties are the substance of the work. **A mount is found through the
node it covers and never through the path it was mounted at.** Prefix matching is
the obvious alternative and it fails in three ways that have no remedy within it,
each of them silent: a symbolic link whose target crosses a mount point is not a
path any prefix describes; `..` leaving a mounted volume must arrive at the
parent of the mount point and a prefix has no way to know it has left; and one
directory reached by two routes would be matched against one prefix and not the
other. **A file reached twice is one node**, which is a correctness requirement
and not a convenience: two copies of a file's description mean that a write
through one which extends the file leaves the other holding the old size and the
old block pointers, and the next write through that one restores them, truncating
the file and orphaning every block the first allocated, silently. And **a volume
opened for writing is marked as not cleanly unmounted before anything else is
written to it**, so that a machine which stops while it is open leaves that mark
behind and the next mount finds a volume it must refuse to write until a check
has been run. A kernel that marked the volume upon unmounting would record only
the mounts that ended well, which are exactly the ones that need no record.

The layer refuses to destroy a file that anything still holds, and refuses to
withdraw a volume upon which anything is still held. POSIX would keep an unlinked
file alive until its last descriptor closed; that requires a list of files with no
name that are not yet gone, and this kernel has none, so the alternative to
refusing is to free the inode and the blocks beneath a descriptor still reading
them.

The root volume of a machine this kernel is booted upon is mounted **read-only**
unless the operator chose the GRUB entry that permits writing, a kernel that
marked a stranger's disk as unclean merely by having been booted imposing a real
cost for nothing.

Corroborating this against `mke2fs` images found a defect in sub-task 5.7:
`i_dtime` is the deletion time of a freed inode and the *link to the next inode*
of one upon the orphan list, and a check distinguishes the two by magnitude, so
the constant 1 this kernel recorded for want of a clock made `e2fsck` report every
inode it had ever freed as the member of a corrupted orphan list. It is now
`UINT32_MAX`, which no volume can hold as an inode number.
...
merely built. The global descriptor table holds seven descriptors in eight slots:
the user's 64-bit code segment, its writable data segment, and a
compatibility-mode code segment this kernel will never execute in and cannot
omit, because `SYSRET` finds the 64-bit descriptor at that one's selector plus
sixteen. The order of the three is not a preference of this kernel's but
arithmetic the processor performs, and it is asserted as an ordering for the
reason that it has no local symptom: every descriptor may be individually perfect
and the transition still fail. A task state segment names the stack the processor
loads upon a transfer to privilege level 0 and, in the first interrupt stack
table entry, a separate stack the double fault is delivered upon — the fault
whose commonest cause is that the stack in use was the thing that went wrong, and
which without a stack of its own would not raise a third exception but would shut
the processor down. Its I/O map base is placed one byte beyond the segment limit,
which is what denies every port to user mode; a base within the limit would have
whatever bytes happened to lie there decide which ports a user program could
drive, a clear bit granting rather than denying. `IA32_STAR`, `IA32_LSTAR` and
`IA32_FMASK` are written and `IA32_EFER.SCE` set, in that order, so there is no
moment at which `SYSCALL` is legal and transfers to address zero.

Two things distinguish this from a configuration merely written down. The
interrupt stack table is **exercised**: a spare vector is raised with the entry
and without it, and the address at which its trap frame was built — the frame
being the first thing placed upon whatever stack the processor selected — must
lie within the double-fault stack in the one case and outside it in the other.
And the transition itself is **exercised**, upon the observation that `SYSCALL`
may be executed from privilege level 0: it raises no privilege, there being none
to raise, but it performs every other part of the transition, so the whole
mechanism is provable now with no user program, no user mapping and no user
stack. It is executed twice, once with the interrupt flag set, because the
assertion that `IA32_FMASK` cleared the flag says nothing whatever if the flag
was already clear. The self-test was then confirmed capable of failing before it
was trusted, by removing the interrupt flag from the mask and observing exactly
the two assertions that bear upon it report.

The next work is sub-task 6.2, which replaces the provisional entry point with
the real one — the stack switch the placeholder deliberately omits, the dispatch
table, and the validation of a user's arguments — and whose system calls are the
operations of the Phase 5 filesystem layer with those arguments copied in.
...
| ---- | -------------- | ------- |
| 2026-09-03 | Phase 6 | Sub-task 6.1 completed, opening the phase: the apparatus a privilege transition is performed out of. Everything this kernel had done it had done at privilege level 0, in one address space, upon one stack; a user program is none of those, and the processor will not carry it across the boundary upon request — the transition is performed by hardware, out of structures the hardware reads directly, and the failure of any of them is a general-protection exception or a reset rather than a return code. Three structures were built and, more to the point, proved. **The descriptors.** The global descriptor table now holds seven descriptors in eight slots, gaining a 64-bit user code segment, a writable user data segment and a compatibility-mode user code segment this kernel will never execute in. The last is not optional and the order is not free: `IA32_STAR` holds two selectors and the processor derives four from them by arithmetic — `SYSCALL` takes `CS` from `[47:32]` and `SS` from that plus eight, `SYSRET` with a 64-bit operand size takes `CS` from `[63:48]` plus sixteen and `SS` from that plus eight — so the compatibility-mode descriptor occupies the slot `[63:48]` itself names, and a slot left empty there is a descriptor `SYSRET` would load and the processor reject. The descriptors are therefore placed by the processor's arithmetic and not by any preference of this kernel's, which is the fact with no local symptom: every descriptor may be individually perfect and the transition still fail, because what `SYSRET` loads is decided by adding sixteen to a number and not by which descriptor anybody meant. The self-test asserts the ordering *as an ordering*, and decodes each descriptor field by field rather than comparing it against a whole quadword, a constant agreeing with a mistaken descriptor as readily as with a correct one. **The task state segment.** In 64-bit mode the structure holds no task state, hardware task switching not existing there; what remains is 104 bytes of stack pointers the processor reads when it needs a stack it can trust. `RSP0` covers the entry from user mode. The interrupt stack table covers the case `RSP0` cannot: an entry named by a gate is loaded *unconditionally*, whatever the privilege level and whether or not it changes, whereas `RSP0` is loaded only upon a change from level 3 — so a fault taken at level 0 upon a stack that is itself the fault gets no new stack from `RSP0`, is pushed onto the broken stack it was already using, and a processor that cannot push an exception frame does not raise a third exception but shuts down. The double fault is given entry 1 for exactly that reason. The page fault is deliberately given none and the self-test asserts its absence: an interrupt stack table entry is a fixed address that does not nest, and the page-fault handler is taken often and may itself fault, so it must run upon a stack that does. The I/O map base is set to 104, one byte beyond the limit of 103, which is what denies every port to user mode; were it within the limit the processor would read the bytes it addressed as a permission bitmap and whatever lay there would decide which ports a user program could drive — a zero left by accident is not a safe default, a clear bit granting the port. The self-test asserts the relation and not the number. The structure is `__attribute__((packed))` with a `_Static_assert` upon its size, registered in `CODING-STANDARDS.md`, Section 7: it is a structure *the processor* reads at fixed offsets, so the field-by-field decoding Section 7.1 prefers for an on-disk format has no meaning here, this kernel not being the reader. **The three registers.** `IA32_STAR`, `IA32_LSTAR` and `IA32_FMASK` are written and `IA32_EFER.SCE` set, in that order, so there is no window in which `SYSCALL` is legal and `IA32_LSTAR` holds address zero. Support is established first by `CPUID`, leaf `0x80000000` queried before leaf `0x80000001` because a processor that does not implement the latter returns the contents of some other leaf rather than an error, and arbitrary data may well have bit 11 of `EDX` set. `IA32_FMASK`'s six flags are named individually and combined rather than the constant `0x47700` being written down, each being there for a reason of its own, and one of them is a hole and the rest are nuisances: `SYSCALL` performs **no stack switch**, so `RSP` is still the caller's when the first instruction of the handler runs, and an interrupt delivered there is delivered at privilege level 0 onto memory a user program controls. The rest — `TF`, `DF`, `NT`, `AC` and both bits of `IOPL` — are single-stepping the kernel on a caller's behalf, an ABI violation, an `IRET` that would attempt a task switch, half of what makes `SMAP` work, and an I/O privilege level inherited from whoever called. The entry point installed is provisional and two of its properties would be defects in the real one: it does not switch stacks, which is what the real path's first act must do, and it does not return by `SYSRET`, which returns to privilege level 3 unconditionally and would drop the self-test into user mode with nothing there to execute; it returns by restoring `R11` and jumping to `RCX` instead. Sub-task 6.2 replaces it. The apparatus is established after `ExceptionInitialise` and not beside the descriptor table it extends, because `LTR` reads the descriptor and faults where it is malformed, and that fault raised before any gate existed would have escalated to a reset — the diagnosis available to whoever met it being a machine that reboots. This is the lesson of `INTERRUPTS.md`, Section 5.1, from its other side: such a structure must be established where its rejection can be reported, as well as remain mapped for as long as the processor may read it. Forty-eight assertions, and the two that matter most do not inspect a structure at all, inspection establishing only what this kernel wrote. The interrupt stack table is exercised by giving a spare vector the double fault's entry, raising it with the entry and without, and comparing where the trap frame was built — the frame being the first thing placed upon whatever stack the processor selected, and nothing else in the machine observing that selection, the stack pointer the frame *records* being the one in use beforehand and identical either way. Raised twice because one measurement proves nothing: a frame that happened to fall in range would satisfy it, and the pair establishes that the entry is what caused the difference. The double fault itself cannot be raised, its handler being fatal by design. And the transition is exercised upon the observation that `SYSCALL` may be executed from privilege level 0: it raises no privilege, there being none to raise, but it loads the selectors, transfers to `IA32_LSTAR`, saves `RCX` and `R11` and applies the mask, so the whole mechanism is provable with no user program, no user mapping and no user stack — and the descriptor type changing from 9 to 11, which only the processor writes, is the single piece of evidence that `LTR` was accepted rather than that a number was written into a register. Executed twice, the second time with the interrupt flag deliberately set, because the assertion that the flag was cleared upon entry says nothing whatever if it was already clear. The self-test was then confirmed capable of failing before it was trusted: `RFLAGS_INTERRUPT_ENABLE` was removed from the mask, exactly the two assertions bearing upon it reported — the one upon the register and the one upon what the processor did — and the run ended `Privilege self-test FAILED.` Corroborated upon a second hypervisor: under VirtualBox, with a different descriptor table handed over by a different firmware and a different memory map beneath it, the report reads value for value as it does under QEMU and the self-test passes. That run also established that this kernel does not detect VirtualBox's 16550A and therefore has no serial channel there, so the automated assertion cannot be made under it and a verdict must be read from the console by pausing the machine while the line is still upon the screen; that is a pre-existing property of the machine, unrelated to this sub-task, and it is now recorded in `docs/project/TESTING.md`, Sections 9.1 and 9.2, together with the procedure for reaching a `VBoxManage` that is a Windows program and understands no WSL2 path. `docs/design/PRIVILEGE.md` added and records the design; `docs/project/TESTING.md` gained Section 13 with the negative test's procedure and Sections 9.1 and 9.2 upon what VirtualBox does and does not permit to be observed, the test record being renumbered to 14; `docs/project/REFERENCES.md` gained the AMD64 Programmer's Manual and eleven further sections of the Intel manual, `SYSCALL`/`SYSRET` moving out of the forward-looking table; `docs/project/CODING-STANDARDS.md` registered the packed and aligned attributes and the distinction between a structure the processor reads and one read from a medium; `docs/design/INTERRUPTS.md` had its forward reference to this sub-task discharged and its second limitation revised; `docs/design/ARCHITECTURE.md`, `docs/design/README.md` and `kernel/README.md` were brought up to date. |
| 2026-09-02 | Phase 5 | Sub-task 5.8 completed, and with it the phase. A virtual filesystem layer, and an EXT2 root volume mounted through it. Sub-tasks 5.1 to 5.7 built an implementation that could do everything the format admits and that could not be used, and the three things it lacked are absent together rather than by coincidence, each being a thing the format has no business knowing: nothing was retained, so a caller wishing to read two files read the superblock twice; nothing was open, so a file was acted upon by naming it and there was no position that advanced, and two readers of one file could not exist without each keeping its own offset and size; and there was one volume, `Ext2ResolvePath` beginning at inode 2 of the device it is given, so a machine with two disks had two unrelated trees and no path that reached both. A filesystem is now registered under a name and supplies sixteen operations, of which four — mount, unmount, read_node and lookup — are not optional because without them nothing can be reached at all, and the absence of any other is refused where it is called rather than by a call through a null pointer, which is what will admit a read-only filesystem or one without directories later without a change to the layer. The binding of EXT2 to it is a file of its own and is the only file in the project that knows both that a node has a reference count and that an inode has fifteen block pointers; putting the declaration in `ext2.h` would have made every consumer of the format compile against the layer as well, and the direction of the dependency would no longer be legible from the includes. Three properties are the substance of the work and each is a decision that the obvious alternative gets silently wrong. A mount is found through the **node it covers** and never through the path it was mounted at, the path being kept for a report alone: prefix matching appears to work and fails in three ways that have no remedy within it, since a symbolic link whose target crosses a mount point is not a path any prefix describes, `..` leaving a mounted volume must arrive at the parent of the mount point and a prefix has no way to know it has left, and one directory reached by two routes would be matched against one prefix and not the other — in every case the path resolves, to the wrong file. A file reached twice is **one node**, which is a correctness requirement and not a convenience: were each caller to hold a copy of the file's description, a write through one that extended the file would leave the other holding the old size and the old block pointers, and the next write through that copy would restore them, truncating the file and orphaning every block the first had allocated, with nothing to report it; the same identity is what makes a parent's link count correct when a directory is created within it, and the self-test asserts exactly that by reading the root's count before and after. And a volume opened for writing is marked as **not cleanly unmounted before anything else is written to it**, so a machine that stops while it is open leaves the mark behind and the next mount reads a volume it must refuse to write until a check has been run; a kernel that marked the volume upon unmounting would record only the mounts that ended well, which are exactly the ones that need no record. The mark is made by clearing the bit that says the volume was clean and not by setting the bit that says errors were found, those being distinct bits saying different things — a volume merely open is intact, and recording it as faulty would have `e2fsck` report errors upon a sound disk and would erase the record of a disk that genuinely had some. `Ext2WriteSuperblock` gained the mount count, the mount being the first thing that alters it. A node whose last reference goes is released at once rather than retained: this is a table of nodes in use and not a cache of nodes recently used, a retained node being a description of a file that may since have been destroyed and its inode reissued, with nothing here to know it; the cost is the decoding of an inode read twice, the buffer cache absorbing the medium. The layer refuses to destroy a file anything else holds and to withdraw a volume upon which anything is still held — an open descriptor, a node being resolved through, or another volume mounted within it, which are three instances of one rule. POSIX would keep an unlinked file alive until its last descriptor closed; that requires the orphan list `i_dtime` is threaded through and the discipline that empties it after a machine has stopped, and this kernel has neither, so the alternative to refusing is to free the inode and every block of the file beneath a descriptor still reading them and to reissue that inode to another file while the first is still open. Resolution walks the caller's path in place with an explicit length, so the parent of a path is resolved by walking a prefix of that path where it stands and nothing in the layer holds a path buffer at all; a symbolic link is followed by re-entering the resolver, bounded at the eight links POSIX requires an implementation to allow, so the whole cost of following one is two kibibytes of stack. `.` and `..` are not interpreted, every EXT2 directory holding both as ordinary entries — the sole exception being `..` leaving a mounted volume, which is a fact of the tree and not of the volume and which only this layer knows. The root volume of a machine this kernel is booted upon is mounted read-only unless the operator chose the GRUB entry that permits writing: a kernel that mounted a stranger's disk for writing would mark it as not cleanly unmounted merely by having been booted, and every such disk would demand a check before its owner could mount it again, which is a real cost imposed for nothing by the very mechanism meant to protect them. The self-test asserts some ninety properties against two volumes composed in memory, the second being a copy of the first with one field altered so that an assertion can say *which* volume a path reached — two identical volumes would make every crossing assertion vacuous and two wholly different ones would leave it unclear which difference had been detected — and it ends by requiring that no node was left held and no descriptor open, which is the assertion a fixed table must be held to since a resolution that failed to release what it held would exhaust it long before a machine had done any real work and would do so silently until it did. Corroborated against `mke2fs` images at both block sizes this kernel accepts: the kernel mounted one at `/` and listed its root with the inode numbers, types and names `debugfs` gives; a read-only mount left the volume byte for byte as it was, with `dumpe2fs` reporting it clean and a mount count of zero; a writable mount that was never withdrawn left it `not clean` — and not "with errors" — with a mount count of one, which is the mark persisting exactly as intended; 5000 bytes were written to a file through a descriptor and read back through the same descriptor after a seek, crossing a block boundary at both block sizes, and `debugfs` extracted them matching byte for byte; the volume was then withdrawn and reported clean; and `e2fsck -fn` found no error through all five passes in every case. That corroboration found a defect in sub-task 5.7, and it is the class of defect only a tool outside the kernel can see, the operation having reported success and the volume having read back correctly: `i_dtime` is the deletion time of a freed inode and the number of the next inode of one upon the orphan list, that list being threaded through the field rather than given a structure of its own, and a check distinguishes the two meanings by magnitude alone — so the constant 1 this kernel recorded for want of a clock made `e2fsck` report every inode it had ever freed as the member of a corrupted orphan list naming inode 1, upon volumes that were intact. It is now `EXT2_DELETION_TIME_UNKNOWN`, which is `UINT32_MAX`: no volume has an inode numbered above `s_inodes_count`, so it cannot be read as a link, and it is the last second the field can express, which is a defensible way of saying that the moment is not known. `docs/storage/VFS.md` added and records the design; `docs/storage/EXT2.md` gained Section 13.6's account of the deletion time and the addition to Section 12.1 upon what the superblock write maintains, its limitations 1, 3, 6, 9, 15 and 17 being revised and a twenty-third added; `docs/project/TESTING.md` gained Section 12 and its record, the test record being renumbered to 13; `docs/project/REFERENCES.md` gained POSIX.1-2017 and the 1974 UNIX paper; `docs/design/ARCHITECTURE.md`, `docs/README.md`, `docs/storage/README.md`, `kernel/README.md` and `boot/grub/grub.cfg` were brought up to date. |
