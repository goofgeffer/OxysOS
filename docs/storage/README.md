...
| [`EXT2.md`](EXT2.md) | The EXT2 volume: the superblock, the block group descriptor table, the inode with the direct and indirect pointers that name a file's blocks, and the directory entries that turn a name into an inode number, the reading of a file's contents, and the allocation, writing and truncation that alter a volume. | [`../../kernel/fs/ext2.c`](../../kernel/fs/ext2.c) | 5.1 to 5.6 |
