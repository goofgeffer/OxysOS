...
| [`VERSIONING.md`](docs/project/VERSIONING.md) | What a release gets called. One repository, many releases: `Oxys 1`, `Oxys 1.1`, `Oxys Aetos` — which is `Oxys 4` with a name — and editions such as `Oxys Aetos Workspace Edition`, which are varieties of a release rather than successors to it. The first release under it is `Oxys 1 Alpha`, cut on 2026-09-16 at the close of Phase 8 and described by [`RELEASE-1-ALPHA.md`](docs/project/RELEASE-1-ALPHA.md); Section 11.1 plans `Oxys 1 Beta` at sub-task 9.7 and `Oxys 1` at about 11.10. |
