...
claims the display and is what `init` now starts; and sub-task 9.6 added
`terminal`, the emulator that at last puts a shell upon this system's own screen,
with `poll-check` beside it, so that `/bin` holds
twenty-two. See
[`../docs/storage/INITRD.md`](../docs/storage/INITRD.md), and Section 2 of it for
why the twelve `-check` programs are **not** carried there: each exists to make a
...
| [`micro/main.c`](micro/main.c) | Added on 2026-09-16 at the project owner's request, beside sub-task 8.6: the smallest editor that can edit. `micro FILE` loads the file, prints it numbered, and takes `p`, `a`, `i N`, `e N`, `d N`, `w`, `q`, `q!` and `wq`; `e` hands the line back through the C library's line editor to be changed with the arrow keys rather than retyped. A line editor and not a screen editor, because the display interprets no cursor-positioning sequence; the file is held whole and written whole, there being no `lseek`. **Since 2026-09-24 a save is all or nothing**: the lines are written into `FILE.micro-save` beside the file, which takes the file's name only once whole, by `unlink` and `link` — a save that fails part way leaves the file as it was, where it once left it cut at the line that failed. |
...
| [`terminal/main.c`](terminal/main.c) | Sub-task 9.6: the terminal emulator — a window, an ordinary `/bin/sh` beneath it upon a pair of pipes, and the bytes carried between them. The grid and the key translation are the C library's; what is here is the window, the pipes, the `poll` it waits in and the drawing of the rows the grid says have changed. Control-C is a signal it sends to the shell's group, there being no terminal to turn a byte into one. It draws white upon black — two colours of its own, a terminal being the one thing here read for minutes at a time — and paints that ground once at start, there being no fill across the protocol. `../docs/design/TERMINAL.md`. |
| [`files/main.c`](files/main.c) | Sub-task 9.7: the file manager — one directory listed, directories first, a press selecting and a second opening, a directory in the same window and a file in `/bin/view`. It changes nothing. `../docs/design/UTILITIES.md`. |
| [`view/main.c`](view/main.c) | Sub-task 9.7: the text viewer — a file wrapped to its window, scrolled by the keys, and wrapped again with the top row kept when the window is made full. `../docs/design/UTILITIES.md`. |
| [`date/main.c`](date/main.c) | Sub-task 9.7: the date and the time of day as the machine's clock holds them, in ISO 8601's extended form, with no zone because there is none. `../docs/design/UTILITIES.md`. |
...
| [`init-check/main.c`](init-check/main.c) | Sub-task 9.3: asserts that `power` is `EPERM` to a process that is not `init` — for every action, the authority being checked before the action — and that `pause` reports `EINTR` when a signal arrives and delivers it. `../docs/design/INIT.md`. |
| [`config-check/main.c`](config-check/main.c) | Sub-task 9.4: asserts the half of the configuration parser only a program can reach — a file written, read back and found to say what was written; a file that is not there leaving the settings empty — and that the files `/etc` ships carry the keys `init` and the desktop read. That last is the one a parser test cannot make: a key renamed in a program and not in the file leaves a machine that boots to a bare screen with every other test passing. |
| [`poll-check/main.c`](poll-check/main.c) | Sub-task 9.6: asserts `poll` from privilege level 3 — an empty pipe, a written one, the end that writes, a pipe at its end, a blocking wait a child ends, two entries at once, and every refusal — and that `tcgroup` from a child whose standard input is a pipe is `ENOTTY` while the parent at the terminal is not refused. The last is what keeps a shell in a window from taking the terminal from the shell at the keyboard. |
| [`window-check/main.c`](window-check/main.c) | Sub-task 9.2: asserts the client protocol from privilege level 3 — the screen's bounds, a window made and told of the focus, pixels taken and a rectangle outside the content refused, the focus passing between two windows and back, a window the caller does not hold refused, and a wait that sleeps until the kernel's self-test injects a key — and leaves a window standing for its ending to destroy. `../docs/design/WINDOWS.md`. |
...
| [`sh/jobs.c`](sh/jobs.c) | Sub-task 8.7: the job table — each pipeline a process group — the foreground job waited for until it ends or stops, the background job announced and reported at a later prompt, and `jobs`, `fg`, `bg` and `kill`. The shell ignores SIGINT and SIGTSTP and every child puts them back. |
| [`sh/run.c`](sh/run.c) | Sub-task 8.4: the search upon `PATH`, the environment built from the exported variables, the `fork`, the `execve` tried upon each candidate in the child, and the `wait` that turns what the child ended with into a status; since 8.5, the redirections performed in the child between the two, in the order written; since 8.6, the pipeline — one child per command, a pipe between each pair, every end closed where it is not needed, every child collected; since 8.7 every pipeline and program a job of its own group, holding the terminal while it runs in the foreground. Reaches system calls, so not compiled into the kernel image. |
...
| [`signal-check/main.c`](signal-check/main.c) | Sub-task 8.7: asserts the signals, the process groups and `waitpid` from a program and its children — a handler entered and returned through, an ignored signal, SIGKILL and SIGSTOP refused a disposition, a computing child ended by the timer's delivery and a sleeping one by the wake, SIGPIPE, a fault as SIGILL, a group ended by one `kill`, a child stopped, reported once, continued and collected — and ends with the number that failed. |
...
[`../docs/design/LIBC.md`](../docs/design/LIBC.md) gives: nothing
...
records why there is no canonical mode — **or, since 8.5, the
