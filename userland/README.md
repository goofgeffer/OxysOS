...
| [`terminal/main.c`](terminal/main.c) | Sub-task 9.6: the terminal emulator — a window, an ordinary `/bin/sh` beneath it upon a pair of pipes, and the bytes carried between them. The grid and the key translation are the C library's; what is here is the window, the pipes, the `poll` it waits in and the drawing of the rows the grid says have changed. Control-C is a signal it sends to the shell's group, there being no terminal to turn a byte into one. `../docs/design/TERMINAL.md`. |
