...
| [`sh/builtins.c`](sh/builtins.c) | Sub-task 8.3: `cd`, `pwd`, `export` and `exit`, and since 8.5 `help`, `true`, `false` and `unset` — the one unit of the shell's that reaches a system call, and therefore the one not compiled into the kernel image. |
