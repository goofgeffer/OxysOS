...
| [`files/main.c`](files/main.c) | Sub-task 9.7: the file manager — one directory listed, directories first, a press selecting and a second opening, a directory in the same window and a file in `/bin/view`. It changes nothing. `../docs/design/UTILITIES.md`, Section 2. |
| [`view/main.c`](view/main.c) | Sub-task 9.7: the text viewer — a file wrapped to its window, scrolled by the keys, and wrapped again with the top row kept when the window is made full. `../docs/design/UTILITIES.md`, Section 3. |
| [`date/main.c`](date/main.c) | Sub-task 9.7: the date and the time of day as the machine's clock holds them, in ISO 8601's extended form, with no zone because there is none. `../docs/design/UTILITIES.md`, Section 4. |
